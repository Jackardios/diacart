...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return Diacart; });
...
/* harmony import */ var _templates_diacart__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../templates/diacart */ "./src/templates/diacart.js");
/* harmony import */ var _templates_diacart_item__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../templates/diacart-item */ "./src/templates/diacart-item.js");
/* harmony import */ var _templates_diacart_mini__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../templates/diacart-mini */ "./src/templates/diacart-mini.js");
/* harmony import */ var _ObjectsStorage__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./ObjectsStorage */ "./src/components/ObjectsStorage.js");
/* harmony import */ var _utils_helpers__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../utils/helpers */ "./src/utils/helpers.js");
/* harmony import */ var _components_diacartCounterInit__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../components/diacartCounterInit */ "./src/components/diacartCounterInit.js");
...
var defaultOptions = {
  ...
  // templates
  templateTagFunction: lit_html__WEBPACK_IMPORTED_MODULE_0__["html"],
  // The default lit-html html() function is used.
  templateRenderFunction: lit_html__WEBPACK_IMPORTED_MODULE_0__["render"],
  // The default lit-html render() function is used.
  template: _templates_diacart__WEBPACK_IMPORTED_MODULE_2__["default"],
  itemTemplate: _templates_diacart_item__WEBPACK_IMPORTED_MODULE_3__["default"],
  miniTemplate: _templates_diacart_mini__WEBPACK_IMPORTED_MODULE_4__["default"],
  totalPriceTemplate: function totalPriceTemplate(_ref) {
    var options = _ref.options,
        totalPrice = _ref.totalPrice;
    return parseFloat(totalPrice).toFixed(2) + (options.currency || "");
  },
  totalQuantityTemplate: function totalQuantityTemplate(_ref2) {
    var totalQuantity = _ref2.totalQuantity;
...
function () {
  function Diacart() {
    ...
    _defineProperty(this, "add", function () {
      ...
      if (!item && console && console.warn) {
        console.warn("'item' argument is ".concat(item, "!"));
        return false;
      }

      var addedId = false;
      item.quantity = _this.options.itemHasQuantity && item.quantity && item.quantity > 0 ? item.quantity : 1;

      if (_this.options.groupBy) {
        var query = _defineProperty({}, _this.options.groupBy, item[_this.options.groupBy]);

        var id = _this._groupItemsByQuery(query);

        if (id) {
          var storageItem = _this.storage.get(id);

          var updateObject = Object.assign({}, item, {
            quantity: storageItem.quantity + item.quantity
          });
          addedId = _this.storage.update(id, updateObject);
        } else {
          addedId = _this.storage.add(item);
        }

        _this.eventEmitter.emit("add", addedId);
    ...
    _defineProperty(this, "remove", function (id) {
      ...
      var removedObj = _this.storage.remove(id);

      _this.eventEmitter.emit("remove", id);

      return removedObj;
    });

    _defineProperty(this, "update", function (id, updateObj) {
      var updatedId = _this.storage.update(id, updateObj);

      _this.eventEmitter.emit("update", id, updateObj);
    ...
    _defineProperty(this, "clear", function () {
      ...
      _this.eventEmitter.emit("clear");
    });

    _defineProperty(this, "order", function () {
      _this.eventEmitter.emit("order", _this.storage.values());
    });

    _defineProperty(this, "render", function () {
      if (_this._containers.length) {
        var containerCompiled = _this.options.template(_this);

        for (var i = 0; i < _this._containers.length; ++i) {
          _this.options.templateRenderFunction(containerCompiled, _this._containers[i]);
        }
      }

      if (_this._miniContainers.length) {
        var miniContainerCompiled = _this.options.miniTemplate(_this);

        for (var _i = 0; _i < _this._miniContainers.length; ++_i) {
          _this.options.templateRenderFunction(miniContainerCompiled, _this._miniContainers[_i]);
        }
      }

      if (_this._totalPriceContainers.length) {
        var totalPriceCompiled = _this.options.totalPriceTemplate(_this);

        for (var _i2 = 0; _i2 < _this._totalPriceContainers.length; ++_i2) {
          _this.options.templateRenderFunction(totalPriceCompiled, _this._totalPriceContainers[_i2]);
        }
      }

      if (_this._totalQuantityContainers.length) {
        var totalQuantityCompiled = _this.options.totalQuantityTemplate(_this);

        for (var _i3 = 0; _i3 < _this._totalQuantityContainers.length; ++_i3) {
          _this.options.templateRenderFunction(totalQuantityCompiled, _this._totalQuantityContainers[_i3]);
        }
      }

      _this.eventEmitter.emit("render");
    });
  ...
  _createClass(Diacart, [{
    ...
    value: function init() {
      ...
      this.storage = new _ObjectsStorage__WEBPACK_IMPORTED_MODULE_5__["default"](this.options.name);
      this.storage.eventEmitter.on("update", function (newValue, prevValue) {
        _this2.eventEmitter.emit("storageUpdate", newValue, prevValue);

        _this2.render();
      });
      this._containers = document.querySelectorAll(this.options.containerSelector);
      this._miniContainers = document.querySelectorAll(this.options.miniContainerSelector);
      this._totalPriceContainers = document.querySelectorAll(this.options.totalPriceContainerSelector);
      this._totalQuantityContainers = document.querySelectorAll(this.options.totalQuantityContainerSelector);
      this.render();
      ...
      if (this._containers.length) {
        for (var i = 0; i < this._containers.length; ++i) {
          Object(_components_diacartCounterInit__WEBPACK_IMPORTED_MODULE_7__["default"])(this._containers[i]);
        }
      }
    }
  }, {
    key: "on",
    value: function on(eventName, emitted, context) {
      return this.eventEmitter.on(eventName, emitted, context);
    }
  }, {
    key: "once",
    value: function once(eventName, emitted, context) {
      return this.eventEmitter.once(eventName, emitted, context);
    }
  }, {
    key: "removeListener",
    value: function removeListener(eventName, emitted, context) {
    ...
    value: function hasItem(query) {
      return !Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["isEmptyObject"])(this.storage.filter(query));
    } // private
    ...
    value: function _attachEventHandlers() {
      ...
      var self = this;
      Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(document, "click", this.options.orderBtnSelector, function (e) {
        e.preventDefault();

        _this3.order();
      }, true);
      Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(document, "click", this.options.clearBtnSelector, function (e) {
        e.preventDefault();

        _this3.clear();
      }, true);
      Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(document, "click", this.options.addToCartBtnSelector, function (e) {
        e.preventDefault();
        var json = this.getAttribute("data-diacart-item-json");
        var item = JSON.parse(json);
        self.add(item);
      }, true);
      Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(document, "click", this.options.removeFromCartBtnSelector, function (e) {
        e.preventDefault();
      ...
      if (this._containers[0]) {
        ...
        Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(this._containers[0], "change", this.options.quantityInputSelector, quantityInputHandler);
        var timeout;
        Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_6__["addDelegatedEventListener"])(this._containers[0], "keyup", this.options.quantityInputSelector, function (e) {
    ...
    value: function _calculateTotalPrice() {
      ...
      if (this.options.itemHasPrice) {
        this.storage.forEach(function (item) {
          var quantity = _this4.options.itemHasQuantity && item.quantity ? parseInt(item.quantity) : 1;
    ...
    value: function _calculateTotalQuantity() {
      ...
      var totalQuantity = 0;
      this.storage.forEach(function (item) {
        totalQuantity += _this5.options.itemHasQuantity && item.quantity ? parseInt(item.quantity) : 1;
      });
    ...
    value: function _groupItemsByQuery() {
      ...
      if (query) {
        var groupItems = this.storage.filter(query);
        var groupItemsKeys = Object.keys(groupItems);

        if (groupItemsKeys.length) {
          var mainItem = groupItems[groupItemsKeys[0]];

          if (groupItemsKeys.length > 1) {
            var quantity = 0;
            groupItemsKeys.forEach(function (key) {
              quantity += groupItems[key].quantity ? parseInt(groupItems[key].quantity) : 1;
            });
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return ObjectsStorage; });
...
var storeEngine = __webpack_require__(/*! store/src/store-engine */ "./node_modules/store/src/store-engine.js");

var store = storeEngine.createStore([__webpack_require__(/*! store/storages/localStorage */ "./node_modules/store/storages/localStorage.js"), __webpack_require__(/*! store/storages/sessionStorage */ "./node_modules/store/storages/sessionStorage.js"), __webpack_require__(/*! store/storages/cookieStorage */ "./node_modules/store/storages/cookieStorage.js")], [__webpack_require__(/*! store/plugins/update */ "./node_modules/store/plugins/update.js"), __webpack_require__(/*! store/plugins/events */ "./node_modules/store/plugins/events.js")]);

var ObjectsStorage =
/*#__PURE__*/
function () {
  function ObjectsStorage(name) {
    var _this = this;

    _classCallCheck(this, ObjectsStorage);
    ...
    this.name = name;
    this.eventEmitter = new eventemitter3__WEBPACK_IMPORTED_MODULE_0___default.a();
    this._store = store; // this._data = {};

    this._store.watch(name, function (newValue, prevValue) {
      _this._data = newValue;

      _this._onStorageUpdate(newValue, prevValue);
    }); // Synchronization between multiple tabs


    window.addEventListener("storage", function (event) {
      if (event.key === _this.name) {
        var prevStorage = _this._data;
        _this._data = JSON.parse(event.newValue || "[]");

        _this._onStorageUpdate(_this._data, prevStorage);
      }
    }, false);
  }

  _createClass(ObjectsStorage, [{
    key: "_onStorageUpdate",
    value: function _onStorageUpdate(newValue, prevValue) {
      this._keys = null;
    ...
    value: function map(callback) {
      ...
      return this.keys.map(function (key) {
        return callback(_this4.data[key], key);
      });
    }
  }, {
    key: "filter",
    ...
    value: function add(object) {
      ...
      this._store.update(this.name, function (data) {
        data[_this5.newIndex] = object;
      });
    ...
    key: "updateByQuery",
    value: function updateByQuery(query, updateObj) {
      var _this6 = this;
      ...
      this._store.update(this.name, function (data) {
        return _this6.keys.forEach(function (id) {
...
/* harmony import */ var _polyfills_closest__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./polyfills/closest */ "./src/polyfills/closest.js");
/* harmony import */ var _polyfills_closest__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_polyfills_closest__WEBPACK_IMPORTED_MODULE_3__);
/* harmony import */ var _components_Diacart__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./components/Diacart */ "./src/components/Diacart.js");
// styles
 // polyfills
...
/* harmony default export */ __webpack_exports__["default"] = (_components_Diacart__WEBPACK_IMPORTED_MODULE_4__["default"]);
...
function _templateObject4() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-item__price\">", "", "</div>"]);
...
function _templateObject3() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-grid-col diacart-grid-col--auto diacart-item-option\"><div class=\"diacart-item__quantity diacart-counter\"><div class=\"diacart-item-option__title\">", ":</div><div class=\"diacart-item-option__value diacart-counter__container diacart-grid diacart-grid--no-gutter\"><button class=\"diacart-grid__col diacart-grid__col--auto diacart-counter__btn diacart-counter__plus-btn\">+</button> <input class=\"diacart-grid__col diacart-grid__col--flex diacart-counter__input\" type=\"number\" name=\"quantity\" data-diacart-quantity-input data-diacart-item-id=\"", "\" .value=\"", "\"> <button class=\"diacart-grid__col diacart-grid__col--auto diacart-counter__btn diacart-counter__minus-btn\">-</button></div></div></div>"]);
...
function _templateObject2() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-grid-col diacart-grid-col--v-gutter diacart-grid-col--auto diacart-item__image-col\"><a href=\"", "\" title=\"", "\" class=\"diacart-item__image\" style=\"background-image: url('", "');\"></a></div>"]);
...
function _templateObject() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-item\" data-diamodal-item data-diacart-item-id=\"", "\"><div class=\"diacart-grid\">", "<div class=\"diacart-grid-col diacart-grid-col--v-gutter diacart-grid-col--flex diacart-item__text-col\"><div class=\"diacart-grid\"><div class=\"diacart-grid-col diacart-grid-col--flex\"><a href=\"", "\" class=\"diacart-item__title\">", "</a></div><div class=\"diacart-grid-col diacart-grid-col--auto\"><button class=\"diacart-item__remove-btn\" data-diacart-remove-from-cart data-diacart-item-id=\"", "\">", "</button></div></div><div class=\"diacart-item__options diacart-grid diacart-grid--v-end\">", "<div class=\"diacart-grid-col diacart-grid-col--flex\">", "</div></div></div></div></div>"]);
...
/* harmony default export */ __webpack_exports__["default"] = (function (_ref) {
  ...
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject(), id || "", !item.image ? "" : Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject2(), item.link || "", item.name || "", item.image || ""), item.link || "", item.name || "", id || "", options.removeFromCartBtnText || "", options.itemHasQuantity && item.quantity ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject3(), options.totalQuantityText, id || "", item.quantity || 1) : "", options.itemHasPrice && item.price ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject4(), item.price || "", options.currency || "") : "");
});
...
/* harmony import */ var lit_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lit-html */ "./node_modules/lit-html/lit-html.js");
function _templateObject2() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-empty\"><h2 class=\"diacart-empty__title\">", "</h2></div>"]);
...
function _templateObject() {
  var data = _taggedTemplateLiteral(["<div class=\"diacart-wrapper\"><section class=\"diacart\"><div class=\"diacart__header\"><div class=\"diacart-grid\"><div class=\"diacart-grid-col diacart-grid-col--flex diacart__header-col\"><div class=\"diacart__title\">", "</div></div><div class=\"diacart-grid-col diacart-grid-col--auto diacart__header-col", "\"><button class=\"diacart__clear-btn diacart-btn diacart-btn--default diacart-btn--medium\" data-diacart-clear>", "</button></div></div></div><div class=\"diacart__items\">", "<div class=\"diacart__footer", "\"><div class=\"diacart__footer-container\"><div class=\"diacart__total-price\">", "<span class=\"diacart__total-price-accent\">", "</span></div><button class=\"diacart__order-btn diacart-btn diacart-btn--primary diacart-btn--medium\" data-diacart-order>", "</button></div></div></div></section></div>"]);
...
/* harmony default export */ __webpack_exports__["default"] = (function (_ref) {
  var options = _ref.options,
      ...
      totalPrice = _ref.totalPrice;
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject(), options.title || "", !storage.length ? " diacart-hidden" : "", options.clearBtnText || "", !storage.length ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject2(), options.emptyCartText) : storage.map(function (item, id) {
    return options.itemTemplate({
      id: id,
      item: item,
      options: options
    });
  }), !storage.length ? " diacart-hidden" : "", options.totalPriceText ? options.totalPriceText + ": " : "", options.totalPriceTemplate({
    options: options,
...
/*!******************************!*\
  !*** ./src/utils/helpers.js ***!
  \******************************/
/*! exports provided: generateUID, createElementFromHTML, addClass, removeClass, ready, addDelegatedEventListener, triggerEvent, isEmptyObject */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
